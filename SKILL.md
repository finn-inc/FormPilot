...
保存先: `~/.claude/form-submit/screenshots/{企業名}/before.png`

企業名にディレクトリ名として使えない文字（`/` 等）が含まれる場合は `_` に置換する。
...
1. `config.options.screenshotAfterSubmit` が `true` の場合は `browser_take_screenshot` でスクリーンショットを撮影する
   保存先: `~/.claude/form-submit/screenshots/{企業名}/after.png`（企業名にディレクトリ名として使えない文字が含まれる場合は `_` に置換する）
